...
const filters = [
  ...
  { id: 'derpy' as FilterType, label: 'Derpy Faces', count: galleryImages.filter(img => img.category.includes('derpy')).length },
  { id: 'royal' as FilterType, label: 'Royal Pose', count: galleryImages.filter(img => img.category.includes('royal')).length },
  { id: 'action' as FilterType, label: 'Action Shots', count: galleryImages.filter(img => img.category.includes('action')).length },
];
...
export function CorgiGallery() {
  ...
  const handleFilterChange = (filter: FilterType) => {
    ...
    gaEvent('corgi_gallery_filter', {
      section: 'gallery',
      filter,
      total_visible: galleryImages.filter(img => 
        filter === 'all' || img.category.includes(filter)
      ).length,
    });
  };
  ...
  const navigateCarousel = (direction: 'next' | 'prev') => {
    ...
    if (direction === 'next') {
      ...
      newIndex = selectedImageIndex === 0 ? filteredImages.length - 1 : selectedImageIndex - 1;
    }
      ...
      <div className="container mx-auto px-4">
        <div className="text-center mb-12 animate-fade-in-up">
          ...
          <p className="text-lg text-muted-foreground max-w-2xl mx-auto">
            A curated collection of the most adorable corgi moments. 
