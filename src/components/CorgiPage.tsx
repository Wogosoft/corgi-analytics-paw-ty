...
export function CorgiPage() {
  ...
  useEffect(() => {
    ...
    const handleKeyPress = (event: KeyboardEvent) => {
      ...
      switch (event.key.toLowerCase()) {
        case 'b':
          // Trigger bark
          const barkButton = document.querySelector('[data-ga="bark_button"]') as HTMLButtonElement;
          if (barkButton) {
            barkButton.click();
            gaEvent('corgi_keyboard_shortcut', {
              key: 'b',
              action: 'bark',
              timestamp: Date.now(),
            });
        ...
        case 't':
          // Give treat
          const treatButton = document.querySelector('[data-ga="treat_button"]') as HTMLButtonElement;
          if (treatButton) {
            treatButton.click();
            gaEvent('corgi_keyboard_shortcut', {
              key: 't',
        ...
        case 'g':
          // Generate name
          const generateButton = document.querySelector('[data-ga="generate_name"]') as HTMLButtonElement;
          if (generateButton) {
  ...
  const copyEmoji = (emoji: string) => {
    navigator.clipboard.writeText(emoji).then(() => {
      ...
      notification.className = 'fixed top-4 left-1/2 transform -translate-x-1/2 bg-primary text-primary-foreground px-4 py-2 rounded-lg shadow-lg z-50 animate-fade-in-up';
    ...
    <div className="min-h-screen bg-background">
      ...
      <div className="fixed bottom-16 left-4 opacity-30 hover:opacity-80 transition-opacity text-xs text-muted-foreground z-40">
        <div className="bg-card/90 backdrop-blur-sm p-2 rounded border text-center">
          <div>Press <kbd className="bg-muted px-1 rounded">B</kbd> to bark</div>
          <div>Press <kbd className="bg-muted px-1 rounded">T</kbd> for treat</div>
          <div>Press <kbd className="bg-muted px-1 rounded">G</kbd> to generate name</div>
        </div>
      </div>
    </div>
  );
